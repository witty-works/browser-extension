const WITTY_EXTENSION_ID = 'hokifickgkhplphjiodbggjmoafhignh';
window['_docs_annotate_canvas_by_ext'] = WITTY_EXTENSION_ID;
...
const langCouldBeDetermined = () => {
  return (
    document.documentElement.getAttribute('witty-could-determine-lang') ===
    'true'
  );
};
...
CanvasRenderingContext2D.prototype.lineTo = function (t, s) {
  if (
    !this.strokeStyle ||
    wittyIsDisabled() ||
    !langCouldBeDetermined() ||
    !GDOCS_LINE_COLORS.includes(this.strokeStyle.toLowerCase())
  ) {
...
CanvasRenderingContext2D.prototype.fillRect = function (t, e, n, r) {
  if (
    ...
    wittyIsDisabled() ||
    !langCouldBeDetermined() ||
    !GDOCS_RECT_COLORS.includes(this.fillStyle.toLowerCase())
  ) {
