...
export const useLog = (componentName: string): (message: string, type?:string, data?: any) => void => {
  const wittyVersion = browser.runtime.getManifest().version;

  const log = (message: string, type?:string, data?: any) => {
    if (DEV_ENV){
      console.log(`%c[Witty v${wittyVersion}]%c[Component: ${componentName}] %c${message}`
        ,`color: ${Colors.blue}`
        ,`color: ${Colors.green}`
