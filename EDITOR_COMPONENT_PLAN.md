...
6. **Firefox MV3 migration: not part of this plan — a standalone track, unblocked but unforced.** Assessed 2026-08-08 against current facts. What changed since this question was last interesting: (a) Mozilla has repeatedly committed to supporting MV2 indefinitely — there is no deadline and AMO keeps accepting MV2, so nothing forces this; (b) the historical dealbreaker is gone — since Firefox 127, MV3 `host_permissions` and content-script origins are shown at install and granted, so the out-of-box "works everywhere" experience survives MV3; (c) Firefox MV3 runs *event pages*, not service workers, and the manifest's existing dual `background.service_worker`+`scripts` keys are exactly the documented cross-browser MV3 pattern — meaning our manifest is already most of the way there, and the background code (already service-worker-disciplined for Chrome) is strictly easier to run in an event page. The actual migration is small: flip `__firefox__manifest_version`, delete the four `__firefox__` overrides (permissions/CSP/browser_action/applications→browser_specific_settings), bump gecko `strict_min_version` to ≥127, later delete the `browser.browserAction` fallbacks. Why it still stays out of this plan: it is orthogonal to the component work (nothing in Phases 0–5 depends on the Firefox manifest version); the Firefox build has **no automated coverage** (Playwright cannot load Firefox extensions — the e2e suite is Chrome-only), so verification is manual QA precisely when the plan wants the suite as its safety net; and it carries store-side risk that must not entangle the restructuring (AMO review of the version flip, permission-prompt behavior for *existing* users on MV2→MV3 update — verify whether granted origins carry over before shipping, plus AMO's newer data-collection-disclosure requirements). Do it as its own single PR whenever convenient — a natural slot is right after Phase 2 lands — with a manual Firefox QA checklist (install fresh, update from current MV2 version, OAuth flow, popover, keyboard shortcut, badges). Revisit *urgently* only if Mozilla ever announces an MV2 timeline.
