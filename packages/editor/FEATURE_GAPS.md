...
- [ ] **Bundle size**: 293 kB gzipped (2.5.0). Aliasing `react`/`react-dom` to `preact/compat` in the editor's build only, with no source changes, gives 244 kB (-17%), and the editor's browser tests pass on it; see this branch's draft PR. chroma-js comes in through `source/shared/DOMutils.ts`.
