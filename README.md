...
The extension and the editor share one version, set in `package.json`, `packages/editor/package.json` and `source/manifest.json` by:
...
Commit the change, merge it, and push a tag with the same name from `dev`:
